...
echo "  ansible-playbook ansible/playbooks/data-setup.yml  --ask-vault-pass                 # Data Services (PostgreSQL + Redis)"
echo "  ansible-playbook ansible/playbooks/ci-setup.yml  --ask-vault-pass                   # CI/CD (ArgoCD + Tekton)"
echo "  ansible-playbook ansible/playbooks/security-setup.yml  --ask-vault-pass             # Security (Kyverno)"
echo "  ansible-playbook ansible/playbooks/backup-setup.yml  --ask-vault-pass               # Backup (Velero)"
echo "  ansible-playbook ansible/playbooks/observability-cluster-setup.yml --ask-vault-pass # Observability K8s"
echo "  ansible-playbook ansible/playbooks/domain-setup.yml  --ask-vault-pass               # HTTPS sottodomini"
echo ""
