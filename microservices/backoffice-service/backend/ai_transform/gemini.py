...
5. Estrai il prezzo da section.events_dati o info_extra.info_e_costi (offers.price, priceCurrency EUR)
...
9. Estrai contatti da info_extra.info_e_contatti:
...
def _call_gemini(
    ...
    """Chiama l'API Gemini e restituisce il JSON Schema.org."""
...
def _call_groq(
    ...
    """Chiama l'API Groq (compatibile OpenAI) e restituisce il JSON Schema.org."""
    with tracer.start_as_current_span("ai.transform_event") as span:
...
def transform_batch(
    ...
    with tracer.start_as_current_span("ai.transform_batch") as batch_span:
        ...
        for i, event in enumerate(events):
            try:
                schema_event = transform_event(event, model, thinking)
